...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const startArenaBtn = document.getElementById('startArenaBtn'); // 🚨 CAMBIADO
    ...
    const exitArenaBtn = document.getElementById('exitArenaBtn'); // 🚨 CAMBIADO
    ...
    const enableStartButton = () => {
        startArenaBtn.disabled = !(selectedAttack && selectedDefense); // 🚨 CAMBIADO
    };
    ...
    const renderCharacterList = () => {
        ...
        if (currentCharacters.length === 0) {
            displayMessage('No tienes personajes disponibles para el combate. Ve al Sanctuary para crear uno.', false); // 🚨 TEXTO AJUSTADO
            return;
        }
    ...
    const handleCharacterSelection = (character) => {
        ...
        // 🚨 CAMBIADO: trainingAttack -> arenaAttack
        renderElementSelections(character, availableAttackElementsDiv, 'arenaAttack', 'initial'); 
        // 🚨 CAMBIADO: trainingDefense -> arenaDefense
        renderElementSelections(character, availableDefenseElementsDiv, 'arenaDefense', 'initial'); 
        ...
        // 🚨 CAMBIADO: trainingAttack -> arenaAttack
        document.querySelectorAll('input[name="arenaAttack"]:checked').forEach(r => r.checked = false);
        // 🚨 CAMBIADO: trainingDefense -> arenaDefense
        document.querySelectorAll('input[name="arenaDefense"]:checked').forEach(r => r.checked = false);
    ...
    const handleElementChange = (e) => {
        ...
        if (name === 'arenaAttack') { // 🚨 CAMBIADO
            selectedAttack = value;
        } else if (name === 'arenaDefense') { // 🚨 CAMBIADO
    ...
    const startCombat = async (atkElement, defElement) => {
        startArenaBtn.disabled = true; // 🚨 CAMBIADO
        repeatCombatBtn.disabled = true;
        displayMessage('Iniciando combate...', false);
        
        try {
            const response = await fetch('/api/arena/start', { // 🚨 RUTA CAMBIADA
                method: 'POST',
            ...
            startArenaBtn.disabled = false; // 🚨 CAMBIADO
            repeatCombatBtn.disabled = false;
        }
    };

    startArenaBtn.addEventListener('click', () => { // 🚨 CAMBIADO
    ...
    const handleMoveSelection = async (playerMove) => {
        ...
        try {
            const response = await fetch('/api/arena/round', { // 🚨 RUTA CAMBIADA
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    gameState: gameState, 
    ...
    exitArenaBtn.addEventListener('click', () => { // 🚨 CAMBIADO
